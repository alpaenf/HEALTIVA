<template>
    ...
    <form @submit.prevent="submit" class="space-y-2">
        <div>
            ...
            <input
                ...
                class="w-full px-3 py-1.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition"
        ...
        <div class="grid grid-cols-2 gap-2">
            <div>
                ...
                <select
                    v-model="form.gender"
                    class="w-full px-3 py-1.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition bg-white"
                >
        ...
        <button
            ...
            class="w-full bg-primary hover:bg-primary-dark text-white font-semibold py-2 rounded-xl
                   transition-all duration-200 disabled:opacity-70 shadow-md shadow-primary/30 mt-1"
        >
    ...
    <a href="/auth/google"
        class="flex items-center justify-center gap-3 w-full border border-gray-200 rounded-xl py-2
    ...
    <p class="text-center text-xs text-gray-500 mt-3">
        Sudah punya akun?
        <Link href="/login" class="text-primary font-semibold hover:underline">Masuk</Link>
    </p>
</template>
...
defineOptions({
    layout: (h, page) => h(AuthLayout, { mode: 'register' }, { default: () => page }),
});
